...
LOCALES = {
    "index.html": dict(
        ...
        desc="탄 노선을 지도에 채워가는 기록장. 일본·한국·뉴욕 — 같은 엔진에 나라마다 다른 노선 데이터를 넣었습니다.",
        home="홈으로",
        kicker="Rail",
        h1="탄 노선을<br>지도에 채워갑니다",
        lead="역을 탭하면 탑승이 기록되고, 탄 노선의 색이 지도 위에 퍼집니다. "
             "같은 엔진 위에 나라마다 다른 노선 데이터를 얹어 만든 세 개의 기록장이에요.",
        cards_label="세 나라",
        how_label="공통점",
        how=[("지도가 기록입니다",
              "체크리스트가 아니라 실제 선로 모양 그대로의 폴리라인이 칠해집니다. 어디까지 갔는지가 한눈에 보여요."),
             ("노선을 다 타면 완주카드",
        ...
        apps=[("테츠로그", "Tetsulog · Japan Rail Trip Log", "/tetsulog/ko/", "tetsulog",
              ...
              ("레일로그", "Raillog · Korea Rail Trip Log", "/raillog/", "raillog",
               "전국 <b>17개 사업자·55개 노선·1,191개 역</b>. 「철도 도장깨기」 톤의 한국판."),
              ("NYC Subway Log", "New York Subway Log", "/nyc-subway-log/ko/", "nyc-subway-log",
               "뉴욕 지하철 <b>25개 노선·475개 역</b>. 첫 도시 지하철판 — Subway Challenge 문화 위에서.")],
        more="자세히 보기 →",
    ),
    "en.html": dict(
        ...
        desc="A log that fills the map with the lines you've ridden. Japan, Korea, New York — "
             "one engine, a different country's network in each.",
        home="Home",
        kicker="Rail",
        ...
        lead="Tap a station to log the ride, and the line's color spreads across the map. "
             "Three logs built on one engine, each carrying a different country's network.",
        cards_label="Three countries",
        how_label="What they share",
        how=[("The map is the record",
              "Not a checklist — the actual track geometry fills in, so how far you've gotten reads at a glance."),
        ...
        apps=[("Tetsulog", "Japan Rail Trip Log", "/tetsulog/en/", "tetsulog",
              ...
              ("Raillog", "Korea Rail Trip Log", "/raillog/en/", "raillog",
               "<b>17 operators, 55 lines, 1,191 stations</b> across Korea."),
              ("NYC Subway Log", "New York Subway Log", "/nyc-subway-log/", "nyc-subway-log",
               "<b>25 services, 475 stations</b> — the first city edition, built on the Subway Challenge.")],
        more="Learn more →",
    ),
    "ja.html": dict(
        lang="ja", label="日本語",
        title="鉄道記録アプリ — 鉄ログ・Raillog・NYC Subway Log",
        desc="乗った路線で地図を埋めていく記録帳。日本・韓国・ニューヨーク — 同じエンジンに、国ごとの路線データを。",
        home="ホームへ",
        kicker="Rail",
        h1="乗った路線で<br>地図を埋めていく",
        lead="駅をタップするだけで乗車が記録され、乗った路線の色が地図に広がります。"
             "同じエンジンの上に、国ごとの路線データを載せた3本の記録帳です。",
        cards_label="3つの国",
        how_label="共通点",
        how=[("地図がそのまま記録に",
              "チェックリストではなく、実際の線路の形どおりのポリラインが塗られます。どこまで行ったかが一目で。"),
             ("路線を完乗すると完乗カード",
        ...
        apps=[("鉄ログ", "Tetsulog · Japan Rail Trip Log", "/tetsulog/", "tetsulog",
              ...
              ("Raillog", "Korea Rail Trip Log", "/raillog/en/", "raillog",
               "韓国全土<b>17社・55路線・1,191駅</b>。韓国版。"),
              ("NYC Subway Log", "New York Subway Log", "/nyc-subway-log/ja/", "nyc-subway-log",
               "ニューヨーク地下鉄<b>25路線・475駅</b>。初の都市地下鉄版 — Subway Challenge 文化の上に。")],
        more="くわしく見る →",
    ),
    "zh-hans.html": dict(
        lang="zh-Hans", label="简体中文",
        title="铁道乘车记录应用 — Tetsulog · Raillog · NYC Subway Log",
        desc="用搭过的线路把地图填满的记录本。日本、韩国、纽约 —— 同一套引擎，装进各国的路网数据。",
        home="回首页",
        kicker="Rail",
        h1="用搭过的线路<br>把地图填满",
        lead="点击车站即可记录乘车，乘坐路线的颜色会在地图上扩散。"
             "同一套引擎之上，装进不同国家路网数据的三本记录簿。",
        cards_label="三个国家",
        how_label="共同点",
        how=[("地图本身就是记录",
              "不是清单，而是按实际线路形状绘制的折线。走到哪里一目了然。"),
             ("坐完整条线就有完乘卡",
        ...
        apps=[("Tetsulog", "日本铁道乘车记录", "/tetsulog/en/", "tetsulog",
              ...
              ("NYC Subway Log", "纽约地铁乘车记录", "/nyc-subway-log/", "nyc-subway-log",
               "纽约地铁<b>25条线路·475个车站</b>。首个城市地铁版本。")],
        more="了解更多 →",
    ),
    "zh-hant.html": dict(
        lang="zh-Hant", label="繁體中文",
        title="鐵道乘車記錄應用程式 — Tetsulog · Raillog · NYC Subway Log",
        desc="用搭過的路線把地圖填滿的紀錄簿。日本、韓國、紐約 —— 同一套引擎，裝進各國的路網資料。",
        home="回首頁",
        kicker="Rail",
        h1="用搭過的路線<br>把地圖填滿",
        lead="點擊車站即可記錄乘車，搭乘路線的顏色會在地圖上擴散。"
             "同一套引擎之上，裝進不同國家路網資料的三本紀錄簿。",
        cards_label="三個國家",
        how_label="共同點",
        how=[("地圖本身就是紀錄",
              "不是清單，而是依實際路線形狀繪製的折線。走到哪裡一目了然。"),
             ("搭完整條路線就有完乘卡",
        ...
        apps=[("Tetsulog", "日本鐵道乘車記錄", "/tetsulog/en/", "tetsulog",
              ...
              ("NYC Subway Log", "紐約地鐵乘車記錄", "/nyc-subway-log/zh-hant/", "nyc-subway-log",
               "紐約地鐵<b>25條路線·475個車站</b>。首個城市地鐵版本。")],
        more="了解更多 →",
    ),
}
